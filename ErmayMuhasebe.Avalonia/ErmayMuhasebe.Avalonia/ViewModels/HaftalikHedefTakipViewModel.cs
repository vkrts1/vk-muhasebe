...
public partial class HaftalikHedefTakipViewModel : ViewModelBase
{
    ...
    private async Task LoadWeeksForYear(int year)
    {
        ...
        foreach (var h in yearTargets)
        {
            var total = salesInvoices
                .Where(f => ISOWeek.GetWeekOfYear(f.Tarih) == h.Hafta && f.Tarih.Year == h.Yil) // Simple check, careful with year boundaries
                .Sum(f => f.GenelToplam);
            
            // Correction: ISOWeek year might differ from Calendar year at boundaries (e.g. Jan 1st can be week 53 of prev year)
            // But strict Yil check on target vs Invoice Year + Week logic should assume invoice belongs to that ISO Week Year.
            // Better: Check if invoice date falls into ISO Week range of Target.
            
            // Using ISO Week logic for invoice matching strictly
            /* 
               var invTotal = salesInvoices.Where(f => ISOWeek.GetYear(f.Tarih) == h.Yil && ISOWeek.GetWeekOfYear(f.Tarih) == h.Hafta).Sum(x => x.GenelToplam);
            */
    ...
    [ObservableProperty] private int _selectedHaftaIndex = 0; // 0-based for specific usage usually, but here we likely bind selected int
