...
import { StyleSheet, Text, View, SafeAreaView, ScrollView, TouchableOpacity, ActivityIndicator, Alert, FlatList, Modal, TextInput, Share, PanResponder } from 'react-native';
...
export default function RaporlarScreen() {
  ...
  const [reportResult, setReportResult] = useState<{ headers: string[]; rows: string[][]; title: string } | null>(null);
  ...
  const handleShareReport = async () => {
    await handleGetPdfReport();
  };

  const handleGetPdfReport = async () => {
    if (!reportResult) return;

    // Rapor PDF'i: mobil rapordan zaten hesaplanan tablo verisini
    // repo'daki Functions PDF API'nin \`/generate/generic\` endpoint'ine gönderir.
    const payload = {
    ...
    Alert.alert('PDF Rapor', 'PDF raporu hazırlanıyor, lütfen bekleyin...');
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList initialNumToRender={20} maxToRenderPerBatch={20} windowSize={5} 
          ...
          renderItem={({ item }) => (
            <TouchableOpacity style={styles.card} onPress={() => handleRunReport(item)}>
              <View style={styles.cardHeader}>
                <View style={[styles.iconBox, { backgroundColor: item.category === 'Stok' ? 'rgba(0, 255, 135, 0.1)' : 'rgba(0, 97, 255, 0.1)' }]}>
                  <FileText color={item.category === 'Stok' ? '#00FF87' : '#0061FF'} size={20} />
                </View>
      ...
      {/* Rapor Sonuç Modalı */}
      <Modal visible={reportResult !== null} transparent={true} animationType="slide">
        <SafeAreaView style={styles.modalOverlay} {...panResponder.panHandlers}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle} numberOfLines={1}>{reportResult?.title}</Text>
              <TouchableOpacity onPress={() => setReportResult(null)}>
                <X color="#FFF" size={24} />
              </TouchableOpacity>
            </View>
...
fs.writeFileSync('e:/avalonia yedek/ermaymuhasebe/ermaymuhasebe-mobil/src/screens/RaporlarScreen.tsx', content);
